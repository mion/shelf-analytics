...
# # # # # # #
# HACK
# This is a temporary version that outputs the masked image with OpenCV
# instead of matplot.
#
def print_detected_result_opencv(image, output_image_path, boxes, masks, class_ids, scores):
    # # # # # #
    # HACK
    calib = {
        "MIN_SCORE": 0.95,
    ...
    for i in range(N):
        ...
        # # # # # #
        # HACK
        # if scores[i] < calib['MIN_SCORE']:
        #     continue
        # bbox_area = BBox(boxes[i], BBoxFormat.y1_x1_y2_x2).area
        # if bbox_area < calib['MIN_BBOX_AREA'] or bbox_area > calib['MAX_BBOX_AREA']:
        #     continue
        # # # # # #
        # Bounding box
        if not np.any(boxes[i]):
...
def detect_humans(model, image, debug_output_path):
    ...
    if debug_output_path is not None:
        # HACK using _opencv version temporarily
        print_detected_result_opencv(image, debug_output_path, boxes, masks, class_ids, scores)
